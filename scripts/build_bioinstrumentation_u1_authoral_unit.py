#!/usr/bin/env python3
"""Build the canonical Bioinstrumentation Unit 1 authoral JSON deterministically."""
...
DEFAULT_SOURCE = ROOT / "data" / "course_redevelopment" / "bioinstrumentacion" / "unit-01-source"
DEFAULT_OUTPUT = ROOT / "data" / "course_redevelopment" / "bioinstrumentacion" / "units" / "unit-01.json"
...
def write_unit(unit: dict[str, Any], output: Path = DEFAULT_OUTPUT) -> None:
    output.parent.mkdir(parents=True, exist_ok=True)
    output.write_bytes(canonical_bytes(unit))
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--check",
        action="store_true",
        help="Fail unless the output already equals the deterministic build.",
    )
...
def main() -> int:
    ...
    if args.check:
        ...
        print(f"OK canonical Bioinstrumentation U1: {args.output.relative_to(ROOT)}")
        return 0
