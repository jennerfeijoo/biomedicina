...
class ImagenesBiomedicasCanonicalCourseTests(unittest.TestCase):
    @classmethod
    ...
    def test_course_is_complete_but_human_review_remains_pending(self) -> None:
        self.assertEqual(self.course["id"], "imagenes-biomedicas")
        self.assertEqual(self.course["code"], "IMGBIO")
    ...
    def test_six_units_are_present_and_disciplinarily_distinct(self) -> None:
        self.assertEqual(len(self.units), 6)
        self.assertEqual(
            [unit["title"] for unit in self.units],
            [
                "Formación y representación de imagen",
                "Radiografía y tomografía",
                "Resonancia magnética",
                "Ultrasonido",
                "Medicina nuclear y óptica",
                "Calidad, artefactos y seguridad",
            ],
        )
        all_text = json.dumps(self.units, ensure_ascii=False).casefold()
        self.assertNotIn(GENERIC, all_text)
        for concept in ("muestreo", "atenuación", "magnetización", "doppler", "pet", "mtf"):
            self.assertIn(concept, all_text)

    def test_each_unit_has_topics_examples_activities_and_assessment(self) -> None:
    ...
    def test_sources_glossary_and_claims_are_traceable(self) -> None:
        ...
        claim_ids = {claim["id"] for claim in self.claims["claims"]}
        self.assertGreaterEqual(len(claim_ids), 24)
    ...
    def test_editorial_boundary_is_explicit(self) -> None:
        ...
        self.assertIn("autorización para operar equipos", notice)
