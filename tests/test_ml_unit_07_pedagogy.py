...
class MachineLearningUnit07PedagogyTests(unittest.TestCase):
    def test_unit_07_keeps_curated_activity_contract(self) -> None:
        unit = json.loads((COURSE / "units" / "unit-07.json").read_text(encoding="utf-8"))
        ...
        self.assertEqual(unit["prerequisite_unit_ids"], [f"MLBIO-U0{i}" for i in range(1, 7)])

    def test_unit_07_assessment_is_explanatory_and_source_backed(self) -> None:
        assessment = json.loads((COURSE / "assessments" / "unit-07.json").read_text(encoding="utf-8"))
