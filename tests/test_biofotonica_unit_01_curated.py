...
class BiofotonicaUnit01CuratedTests(unittest.TestCase):
    ...
    def test_generic_template_marker_is_removed(self) -> None:
        ...
        for concept in ("coeficiente de absorción", "dispersión reducida", "beer–lambert", "cromóforo"):
            self.assertIn(concept, text)

    def test_theory_is_substantive_and_optically_specific(self) -> None:
        ...
        for concept in (
            "atenuación balística",
            "anisotropía",
            "profundidad de interrogación",
    ...
    def test_core_equations_are_present(self) -> None:
        ...
        self.assertIn("\\mu_t=\\mu_a+\\mu_s", equations)
        self.assertIn("\\mu_s'=\\mu_s(1-g)", equations)
        self.assertIn("I(z)=I_0e^{-\\mu_a z}", equations)
        self.assertIn("\\ell_s'=\\frac{1}{\\mu_s'}", equations)
    ...
    def test_guided_activity_is_scaffolded_and_synthetic(self) -> None:
        ...
        self.assertIn("532", text)
        self.assertIn("970", text)
        self.assertIn("no midas personas", text)
    ...
    def test_glossary_examples_errors_and_assessment_are_specific(self) -> None:
        ...
        for term in (
            "absorción",
            "coeficiente de absorción (μa)",
            "factor de anisotropía (g)",
            "coeficiente de dispersión reducida (μs′)",
            "profundidad de interrogación",
        ):
    ...
    def test_sources_are_directly_traceable(self) -> None:
        ...
        self.assertIn("https://pubmed.ncbi.nlm.nih.gov/34713647/", urls)
        self.assertIn("https://pmc.ncbi.nlm.nih.gov/articles/PMC5560608/", urls)
        self.assertIn("https://omlc.org/~prahl/projects/properties.html", urls)

    def test_clinical_and_safety_boundary_is_explicit(self) -> None:
        notice = self.unit["editorial_notice"].casefold()
        purpose = self.unit["purpose"].casefold()
        self.assertIn("no constituye revisión disciplinar externa", notice)
        self.assertIn("validación clínica", notice)
        self.assertIn("seguridad óptica", notice)
        self.assertIn("diagnóstico", purpose)
        self.assertIn("eficacia terapéutica", purpose)
