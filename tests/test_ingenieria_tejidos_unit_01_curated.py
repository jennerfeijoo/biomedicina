# Final user-authored trigger after public and subject-metadata synchronization.
from __future__ import annotations

import json
import unittest
from pathlib import Path

ROOT = Path(__file__).resolve().parents[1]
SOURCE = ROOT / "data" / "course_redevelopment" / "ingenieria-tejidos" / "units" / "unit-01.json"
MIRROR = ROOT / "data" / "generated_units" / "ingenieria-tejidos" / "unit-01.json"
GENERIC = "concepto de la unidad que debe definirse mediante entidades observables"


class IngenieriaTejidosUnit01CuratedTests(unittest.TestCase):
    @classmethod
    def setUpClass(cls) -> None:
        cls.unit = json.loads(SOURCE.read_text(encoding="utf-8"))

    def test_generated_unit_is_exact_redevelopment_mirror(self) -> None:
        self.assertEqual(SOURCE.read_bytes(), MIRROR.read_bytes())
        self.assertEqual(self.unit["schema_version"], "2.0")
        self.assertEqual(self.unit["subject_id"], "ingenieria-tejidos")
        self.assertEqual(self.unit["unit"], 1)
        self.assertEqual(self.unit["status"], "review")

    def test_generic_and_cross_unit_material_are_removed(self) -> None:
        text = SOURCE.read_text(encoding="utf-8").casefold()
        self.assertNotIn(GENERIC, text)
        self.assertNotIn("\\sigma=\\frac{f}{a_0}", text)
        self.assertNotIn("condición de envejecimiento", text)
        self.assertIn("tríada", text)
        self.assertIn("autoorganización", text)
        self.assertIn("microambiente", text)

    def test_theory_is_foundational_and_substantive(self) -> None:
        sections = self.unit["theory_sections"]
        self.assertEqual(len(sections), 4)
        self.assertTrue(all(len(section["paragraphs"]) >= 4 for section in sections))
        self.assertTrue(all(len(section["key_points"]) >= 4 for section in sections))
        theory = " ".join(p for section in sections for p in section["paragraphs"]).casefold()
        for concept in (
            "uso previsto",
            "requisitos verificables",
            "matriz extracelular",
            "señales biomecánicas",
            "viabilidad",
            "arquitectura",
            "función emergente",
            "trazabilidad",
        ):
            self.assertIn(concept, theory)
        self.assertIn("no demuestra integración", theory)

    def test_curricular_boundaries_are_explicit(self) -> None:
        notice = self.unit["editorial_notice"].casefold()
        purpose = self.unit["purpose"].casefold()
        self.assertIn("no constituye revisión disciplinar externa", notice)
        self.assertIn("unidades posteriores", notice)
        self.assertIn("eficacia clínica", purpose)
        self.assertIn("sin tratar la tríada clásica como una receta universal", purpose)

    def test_guided_activities_use_progressive_scaffolding_and_synthetic_cases(self) -> None:
        activities = self.unit["guided_activities"]
        self.assertEqual(len(activities), 3)
        text = json.dumps(activities, ensure_ascii=False).casefold()
        self.assertIn("sintético", text)
        self.assertIn("elimina progresivamente las ayudas", text)
        self.assertIn("necesidad→requisito→medida→criterio", text)
        self.assertIn("versión antes", text)
        total_items = sum(
            len(activity.get(key, []))
            for activity in activities
            for key in ("instructions", "problems", "tasks", "deliverables", "checking_criteria")
        )
        self.assertGreaterEqual(total_items, 50)

    def test_glossary_examples_errors_and_assessment_are_specific(self) -> None:
        self.assertGreaterEqual(len(self.unit["glossary"]), 20)
        self.assertGreaterEqual(len(self.unit["worked_examples"]), 4)
        self.assertGreaterEqual(len(self.unit["common_errors"]), 10)
        self.assertGreaterEqual(len(self.unit["self_assessment"]), 10)
        terms = {entry["term"].casefold() for entry in self.unit["glossary"]}
        for term in (
            "ingeniería de tejidos",
            "matriz extracelular",
            "andamio",
            "estrategia sin andamio",
            "mecanotransducción",
            "trazabilidad de diseño",
        ):
            self.assertIn(term, terms)

    def test_sources_are_traceable_and_directly_verified(self) -> None:
        sources = self.unit["sources"]
        self.assertGreaterEqual(len(sources), 8)
        self.assertTrue(all(item.get("verification_status") == "verified_directly" for item in sources))
        urls = {item["url"] for item in sources}
        self.assertIn("https://pubmed.ncbi.nlm.nih.gov/8493529/", urls)
        self.assertIn("https://pubmed.ncbi.nlm.nih.gov/11834815/", urls)
        self.assertIn("https://pubmed.ncbi.nlm.nih.gov/19556500/", urls)
        self.assertIn("https://pubmed.ncbi.nlm.nih.gov/24845994/", urls)
        self.assertIn("https://www.nature.com/articles/nbt1055", urls)

    def test_in_vitro_evidence_is_not_overclaimed(self) -> None:
        theory = " ".join(p for section in self.unit["theory_sections"] for p in section["paragraphs"]).casefold()
        assessment = json.dumps(self.unit["self_assessment"], ensure_ascii=False).casefold()
        self.assertIn("seguridad, beneficio clínico", theory)
        self.assertIn("no puede concluirse", assessment)
        self.assertIn("clínicamente beneficioso", assessment)


if __name__ == "__main__":
    unittest.main()
