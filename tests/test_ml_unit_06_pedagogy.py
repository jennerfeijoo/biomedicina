...
class MachineLearningUnit06PedagogyTests(unittest.TestCase):
    def test_unit_06_keeps_curated_activity_contract(self) -> None:
        unit = json.loads((COURSE / "units" / "unit-06.json").read_text(encoding="utf-8"))
        ...
        self.assertEqual(unit["prerequisite_unit_ids"], [f"MLBIO-U0{i}" for i in range(1, 6)])

    def test_unit_06_assessment_is_explanatory_and_complete(self) -> None:
        assessment = json.loads((COURSE / "assessments" / "unit-06.json").read_text(encoding="utf-8"))
