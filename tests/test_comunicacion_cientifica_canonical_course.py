...
COURSE_ROOT = ROOT / "data" / "courses" / "comunicacion-cientifica"
...
class ComunicacionCientificaCanonicalCourseTests(unittest.TestCase):
    ...
    def test_course_is_complete_without_false_human_review(self) -> None:
        self.assertEqual(self.course["id"], "comunicacion-cientifica")
        self.assertEqual(self.course["code"], "COMCI")
    ...
    def test_six_units_are_present_and_distinct(self) -> None:
        self.assertEqual(
            ...
            [
                "Pregunta, audiencia y propósito",
                "Estructura de argumentos",
                "Escritura académica",
                "Visualización y presentaciones",
                "Divulgación y medios",
                "Integridad, revisión y publicación",
            ],
        )
        text = json.dumps(self.units, ensure_ascii=False).casefold()
        self.assertNotIn(GENERIC, text)
        for concept in ("audiencia", "causalidad", "imrad", "accesibilidad", "riesgo", "peer review"):
    ...
    def test_course_preserves_evidence_boundaries(self) -> None:
        text = json.dumps(self.units, ensure_ascii=False).casefold()
        for distinction in ("significación", "causalidad", "incertidumbre", "revisión", "confidencialidad", "autoría"):
        ...
        self.assertIn("integridad", notice)
