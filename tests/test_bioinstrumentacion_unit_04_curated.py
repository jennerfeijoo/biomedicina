...
class BioinstrumentacionUnit04CuratedTests(unittest.TestCase):
 ...
 def test_theory_and_scope(self):
  self.assertEqual(len(self.u["topics"]),4); self.assertEqual(sum(len(t["subtopics"]) for t in self.u["topics"]),16); self.assertEqual(len(self.u["examples"]),3); self.assertEqual(self.u["status"]["sources"],"traceable"); self.assertEqual(self.u["status"]["content"],"in_review"); self.assertEqual(self.u["status"]["pedagogy"],"in_review"); self.assertEqual(self.u["status"]["internal_review"],"pending"); self.assertEqual(self.u["status"]["external_review"],"pending"); self.assertEqual(self.u["status"]["publication"],"published_provisional")
  text=json.dumps(self.u,ensure_ascii=False).lower(); self.assertIn("u5 aborda muestreo, cuantización",text); self.assertIn("no tiene un documento autoral histórico equivalente",text)
 ...
 def test_assessment(self):
  ...
  for q in self.a["items"]: self.assertEqual(q["type"],"case_analysis"); self.assertTrue(q["source_ids"]); self.assertTrue(q["answer_key"]["explanation"]); self.assertTrue(q["answer_key"]["common_misconceptions"]); covered.update(q["linked_learning_outcome_ids"])
 ...
 def test_traceability(self):
  ...
  claims=[c for c in self.c["claims"] if c.get("unit_id")=="BIOINST-U04"]; self.assertEqual(len(claims),16); self.assertEqual(len(self.u["claim_ids"]),16); self.assertEqual(self.u["claim_ids"],[c["id"] for c in claims]); serialized=json.dumps(self.u,ensure_ascii=False)
